...
export default {
	...
	methods: {
      ...
      const Dark = this.$cookies.get('dark')
			...
			const H1Page = document.querySelector('h1')
      ...
      if (Dark === 'off') {
          ...
          circle[i].classList.add('pageblack')
				}
				for (let i=0; i<path.length ; i++) {
          path[i].classList.add('pageblack')
				}
				for (let i=0; i<line.length ; i++) {
          line[i].classList.add('pageblack')
				}
				...
				for (let i=0; i<circle.length ; i++) {
          circle[i].classList.remove('pageblack')
				}
				for (let i=0; i<path.length ; i++) {
          path[i].classList.remove('pageblack')
				}
				for (let i=0; i<line.length ; i++) {
          line[i].classList.remove('pageblack')
				}
